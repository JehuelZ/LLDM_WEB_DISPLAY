...
def main():
    print("Fetching roles and admins...")
    profiles = make_request("GET", "profiles?select=name,role,roles,email")
    if profiles:
        admins = [p for p in profiles if (p.get('role') in ['Administrador', 'Admin']) or ('admin' in (p.get('roles') or []))]
        print(f"Found {len(admins)} admins:")
        for a in admins:
            print(f"- {a['name']} ({a['role']}) roles: {a['roles']}")
        
        ministers = [p for p in profiles if p.get('role') == 'Ministro a Cargo']
        print(f"\nFound {len(ministers)} ministers:")
        for m in ministers:
            print(f"- {m['name']} ({m['role']}) roles: {m['roles']}")
