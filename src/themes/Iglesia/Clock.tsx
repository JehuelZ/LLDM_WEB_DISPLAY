...
export function IglesiaProgress({ slides, currentSlide, isPaused }: { slides?: any[], currentSlide: number, isPaused?: boolean }) {
    ...
    return (
        <>
                    ...
                    {!hideExtra && (
                        <div style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingRight: 24, borderRight: `1px solid ${T.borderAccent}` }}>
                                ...
                                <div>
                                    <p style={{ fontSize: 9, fontWeight: 800, color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.12em', fontFamily: T.fontMontserrat, margin: 0 }}>{(settings as any).city || 'Rodeo'}</p>
                                    <p style={{ fontSize: 28, fontWeight: 700, color: T.textPrimary, fontFamily: T.fontInter, margin: 0 }}>24°C</p>
                                </div>
                            </div>
                                    ...
                                    return (
                                        <div key={offset} style={{
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                                {offset % 2 === 0 ? <Sun size={14} color={T.tertiary} /> : <Moon size={14} color={T.secondary} />}
                                                <span style={{ fontSize: 16, fontWeight: 600, color: T.textPrimary, fontFamily: T.fontInter }}>{22 + offset}°</span>
                                            </div>
                                        </div>
                                    );
                                })}
            ...
            {/* Top Static Weekly Theme Banner */}
            {!hideExtra && theme?.title && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <BookOpen style={{ color: T.accent }} size={22} />
                        <span style={{
                            fontSize: 12,
                            fontWeight: 800,
                            color: T.accent,
                            ...
                            {themeLabel}: <span style={{ color: T.textPrimary, fontSize: 18, fontWeight: 700, marginLeft: 6 }}>{theme.title}</span>
                        </span>
                    </div>
                </div>
            )}
